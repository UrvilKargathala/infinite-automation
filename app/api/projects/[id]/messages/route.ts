...
import type { Attachment, ProjectMessage } from "@/types";

export const dynamic = "force-dynamic";

function toMessage(r: Record<string, unknown>): ProjectMessage {
  return {
    id: r.id as number,
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const messageRows = await sql`
    SELECT m.id, m.project_id, m.text, m.attachments, m.created_at::text AS created_at,
    ...
    LEFT JOIN project_messages rt ON rt.id = m.reply_to_id
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const rows = await sql`
    INSERT INTO project_messages (project_id, user_id, text, attachments, reply_to_id, is_forwarded)
    VALUES (${projectId}, ${me.id}, ${text}, ${JSON.stringify(attachments)}, ${replyToId}, ${isForwarded})
  ...
  if (replyToId) {
    const rt = await sql`
      SELECT m.text, u.full_name FROM project_messages m JOIN users u ON u.id = m.user_id WHERE m.id = ${replyToId}
    `;
    if (rt.length > 0) {
